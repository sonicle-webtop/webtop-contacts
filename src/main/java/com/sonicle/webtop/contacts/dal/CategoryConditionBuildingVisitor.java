...
public class CategoryConditionBuildingVisitor extends JOOQConditionBuildingVisitor {
	...
	protected Condition buildCondition(String fieldName, Operator operator, Collection<?> values) {
		...
		} else if (CategoryQuery.PROVIDER.equals(fieldName)) {
			return defaultCondition(CATEGORIES.PROVIDER, operator, parseProviderValues(values), DefaultConditionOption.stringICaseComparison());
	...
	private Collection<String> parseProviderValues(Collection<?> values) {
		...
		for (Object value : values) {
			...
			final CategoryBase.Provider sync = EnumUtils.forString(svalue, CategoryBase.Provider.class, true);
