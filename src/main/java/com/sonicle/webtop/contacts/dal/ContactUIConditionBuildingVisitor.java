 ...
 * @author malbinola
 */
public class ContactUIConditionBuildingVisitor extends ContactConditionBuildingVisitor {
	
	public ContactUIConditionBuildingVisitor() {
		super();
	}
	
	@Override
	protected Condition buildCondition(String fieldName, Operator operator, Collection<?> values) {
		if (ContactQueryUI.ANY_COMPANY.equals(fieldName)) {
			return defaultCondition(CONTACTS_.COMPANY, operator, values)
				.or(defaultCondition(CONTACTS_.COMPANY_MASTER_DATA_ID, operator, values))
				.or(defaultCondition(CONTACTS_.DEPARTMENT, operator, values))
				.or(defaultCondition(CONTACTS_.FUNCTION, operator, values));
			
		} else if (ContactQueryUI.ANY.equals(fieldName)) {
			return defaultCondition(CONTACTS_.DISPLAY_NAME, operator, values)
				...
				.or(defaultCondition(CONTACTS_.COMPANY, operator, values))
				.or(defaultCondition(CONTACTS_.COMPANY_MASTER_DATA_ID, operator, values))
				.or(defaultCondition(CONTACTS_.WORK_MOBILE, operator, values))
